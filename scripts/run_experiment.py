...
from lieprobe.configs.loader import register_schema
...
def _load_stage_registry(package: str = "lieprobe") -> dict:
...
def _run_stages(cfg: DictConfig, run_dir: Path) -> dict:
    ...
    if unavailable:
        ...
        print(f"      in lieprobe.stages: {unavailable}")
...
def main(cfg: DictConfig) -> None:
    ...
    from lieprobe.configs.loader import config_to_dict, save_resolved_config
    from lieprobe.models.device import get_device
    from lieprobe.utils.logging import configure_logging
    from lieprobe.utils.reproducibility import set_seed
    from lieprobe.utils.run_manifest import create_run_dir, save_run_metadata
