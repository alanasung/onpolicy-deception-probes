<p align="center">
  ...
  <p align="center">SPAR Fall 2026 · <a href="https://github.com/alanasung/onpolicy-deception-probes">Training Deception Probes on On-Policy Lies</a></p>
</p>
...
**Training Deception Probes on On-Policy Lies**. The shared engineering spine (config, caching, hooks, metrics,
...
| Stage | Module | Output |
|---|---|---|
| Compose config | `configs/` + `lieprobe.configs` | resolved `config.yaml` |
| Build data | `lieprobe.data` | splits + manifest |
| Load model | `lieprobe.models` | `LoadedModel` + resolved commit |
| Run stages | `scripts/run_experiment.py` | per-stage JSON |
| Aggregate | `lieprobe.reporting` | `results.json` + tables + figures |
...
├── src/lieprobe/       # installable library (print-free)
...
- SPAR Fall 2026 project posting: https://github.com/alanasung/onpolicy-deception-probes
- Complexity bar: Critical Data PRIMED-AI / RecursiveJEPA engineering standard

## Citation

```bibtex
@misc{lieprobe_spar_f26,
  title        = {Training Deception Probes on On-Policy Lies},
  author       = {Alana Sung},
  year         = {2026},
  howpublished = {SPAR Fall 2026},
  note         = {Train and evaluate deception probes on on-policy model-generated lies rather than off-policy text.},
  url          = {https://github.com/alanasung/onpolicy-deception-probes}
}
```
...
Ablations live under `src/lieprobe/ablation/` and return structured dicts. Presets
