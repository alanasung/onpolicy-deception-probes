# Design Document: Training Deception Probes on On-Policy Lies

## 1. Purpose

This document is the method contract for **Training Deception Probes on On-Policy Lies**. It specifies what is
...
One-liner: Train and evaluate deception probes on on-policy model-generated lies rather than off-policy text.

Hypothesis: Train and evaluate deception probes on on-policy model-generated lies rather than off-policy text.
...
|---|---|---|
| Config schema | `lieprobe.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `lieprobe.configs.loader` | Hydra compose API |
| Device | `lieprobe.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `lieprobe.models.registry` | Pinned revisions + hardware fit |
| Hooks | `lieprobe.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `lieprobe.models.generation` | Chat template path recording |
| Data | `lieprobe.data` | Synthetic items, splits, manifests |
| Evaluation | `lieprobe.evaluation` | Metrics, failure harness, protocols |
| Ablation | `lieprobe.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `lieprobe.reporting` | Aggregate → MD/TeX/figures |
| Cache | `lieprobe.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/onpolicy-deception-probes
...
`src/lieprobe/lieprobe/` are preserved across regeneration.
